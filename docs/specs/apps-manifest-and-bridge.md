...
| 方法 | 路径 | 权限 | 说明 |
|---|---|---|---|
| GET | `/apps` | `manage_options` | `{ "apps": [ <manifest + entitlement_status> ], "index_status": "ok"\|"unreachable"\|"invalid"\|"unconfigured" }`。空目录为 `ok` + `apps: []`；拉索引失败为 `unreachable`；签名或 JSON 无效为 `invalid`。未配置索引源（空 source）为 `unconfigured`。 |
