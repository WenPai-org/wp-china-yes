...
判定：`is_admin()`（含 `admin-ajax` / REST 带 `X-WP-Nonce` 的后台请求视为 admin；前台 REST 视为 frontend；WP-CLI 视为 admin）。
...
| `privacy.data_residency` | 按主机表 | **统筹待定**：服务器在海外时 A 档改道是否仍执行 | 同待定 | 规格任务提出方案；拍板前保持现状（按主机表） |

头像不用单一 `scope` 键：`connectivity.avatar.admin` 与 `connectivity.avatar.frontend` 为两个独立枚举值。旧单值迁移：两个值都等于它。

`admin_assets` 是顶层键 `wpcy_settings.admin_assets`（枚举 `on` \| `off`），不是 `connectivity` 子键。4.0 无运行时行为。
...
- `privacy.data_residency` 在统筹拍板前保持现状（按主机表），本 ADR 不改主机表。
...
- `docs/specs/config-schema.md` 含 `profile`、`connectivity.public_assets.scope`、`connectivity.avatar.admin` / `frontend`、`admin_assets`、`schema_version` const 2 与 `upgrade_1_to_2` 描述；默认矩阵与上表逐格一致。
- `docs/specs/rest-api.md` 含 settings 字段更新与 `GET /profile/suggest`。
- `docs/design/admin-ui-spec.md` §2 有场景条目，§4 含本决定全部界面用词。
