...
| 2026-09-03（晚） | 定稿 §7.5a | 小工具容器进首发；公告模块保留；option 键名去版本号；后台改整站 React + 恢复页 |
| 2026-09-06 | ADR-004 | 站点场景（profile）与连通性作用域（scope）为 4.0 首发必需项；见 `docs/architecture/adr-004-site-profile-and-scope.md` |
...
| 小工具容器 | `Services/Apps` | 文派托管 HTML 小工具在沙箱 iframe 内运行，manifest 签名，桥接 REST，数据落用户站 |
| 公告 | `Admin/Announcements` | 固定源（wptea.com、one.weixiaoduo.com）经 wpcy.com 聚合 JSON，每日一拉，最多 5 条，可逐条关闭，不可加源 |

站点场景（`profile`：国内站 / 跨境 / 外贸站 / 混合站）与连通性作用域（`scope`）是 4.0 首发必需项，见 [ADR-004](architecture/adr-004-site-profile-and-scope.md)。公共库、头像按场景带默认作用域；`admin_assets` 4.0 只预留键与开关位，4.1 才改写。默认矩阵不得改值。
...
| RSS、新闻、欢迎营销区 | 欢迎营销区删除；新闻改为固定源公告模块（§4.1） |
...
建议使用新 option，避免继续扩大 `wp_china_yes`。键名稳定不带版本；演进靠结构内 `schema_version` 与迁移器。

- 单站：`wpcy_settings`，`autoload = yes`；
- Multisite 网络策略：`wpcy_network_settings`；
- 本站覆盖：`wpcy_site_overrides`；
- 云服务身份：`wpcy_site_identity`，`autoload = no`；
- 迁移备份：`wpcy_migration_backup`；
...
设置读写必须经过 Repository + Schema + Validator。业务模块不能直接调用 `get_option()`、`update_option()`。完整 schema 见 `docs/specs/config-schema.md`。

2026-09-06（ADR-004）：`schema_version` 升至 2。新增顶层 `profile`、`admin_assets`；`public_assets` 改为 `{ items, scope }` 对象；`avatar` 拆成 `admin` / `frontend` 两个独立值。升级函数与默认矩阵见 config-schema，不改上例的历史形状。

### 5.5 管理界面：整站 React 应用 + 恢复页

管理界面是一个整站 React 应用，外加一个服务端渲染的恢复页。不再使用旧框架，也不复制任何通用设置框架。专项决策见 ADR-001（修订）与 ADR-002。

**React 应用**

- 技术栈：`@wordpress/components` + `@wordpress/data` + `@wordpress/api-fetch`。
- 列表用 `@wordpress/dataviews`、设置表单用 DataForm、布局用 `@wordpress/admin-ui` 的 `<Page>`（隔离在布局层）、命令面板用 `@wordpress/commands`；**不用 `@wordpress/interface` 做壳**（官方仍标 experimental）；不引入第三方 UI 库。作为普通 `add_menu_page` 页存在，与 OpenStation（`desktop-mode` 插件）兼容但不依赖、不模仿。依据 `linuxjoy docs/research/2026-09-03-wordpress-admin-desktop-and-new-apis.md`。
- 构建：`@wordpress/scripts`；发布包带编译产物；源码进仓。
- token 只覆盖 `--wp-admin-theme-color` 系列与 Phosphor 图标。
- 所有数据经 REST 命名空间 `wpcy/v1`；`wp_localize_script` / `wp_add_inline_script` 只传 nonce、REST root、当前用户能力、初始设置快照。

**恢复页**

- 独立菜单项 `?page=wpcy-recovery`，由 PHP 注册。
- 服务端渲染表单，无 JS 可用。
- 一键关闭全部 URL 改写与所有模块。仅此一页服务端渲染。

四页名称不变：

- 概览（含公告）：当前线路、异常、建议，以及固定源公告模块；
- 连接优化：WordPress.org、公共库、头像、可选字体；
- 文派服务：站点绑定状态、权益与配额、用量、**小工具**、服务入口（一律指向 `https://wpcy.com/go/{service}`，禁止直写第三方域名）；
- 诊断：检查结果、导出、重置。

不再提供选择显示哪些 section、欢迎营销页、产品入口集合和隐藏菜单功能。

首次安装向导的第一步是选择站点场景（国内站 / 跨境 / 外贸站 / 混合站），插件给出建议、用户确认。升级站未选前视为 `domestic`，概览给「确认你的站点场景」提示、不打断。见 [ADR-004](architecture/adr-004-site-profile-and-scope.md)。

整站 React 应用的技术边界、恢复页合同与构建链见 `docs/architecture/adr-002-admin-react-app.md`。
...
- 迁移前保存 `wpcy_migration_backup`，记录旧设置 hash、迁移时间和插件版本，不复制敏感凭据。
...
| `admincdn_public` | `connectivity.public_assets` | `googlefonts`→`google_fonts`，`googleajax`→`google_ajax`，`cdnjs` / `jsdelivr` / `emoji`；与 files/dev 合并去重；未知值 ignored（`unsupported_whitelist`）；三键都缺失用 schema 默认，任一存在且合并为空则 `[]` |
...
| 3.8 `admincdn` 或 3.9 `admincdn_files` 含 `admin` | `admin_assets` | `'on'`（4.0 不改写，4.1 生效）；迁移报告「后台加速：已保留设置，4.1 起生效」；**不再进 ignored**（撤销 M4-02b）。见 [ADR-004](architecture/adr-004-site-profile-and-scope.md) D3 |
| 其他字段 | 不迁移 | 在迁移报告中列出 ignored 原因 |

3.x 数据不足以判断服务器位置 → 一律 `profile=domestic` + 概览提示确认；不猜。`frontend` token、`bootstrapcdn` 仍进 `ignored`。旧头像单值 → `connectivity.avatar.admin` 与 `frontend` 都等于它。
...
- 浏览器：Playwright 覆盖四页 + 恢复页；四个后台页面、无 JS 时恢复页可关闭全部改写、CDN 连接、刷新、断开；
- 合同测试：文派 CDN sandbox 的固定 fixtures；小工具桥接（mock manifest 与 mock 工具页）；
...
- React 后台应用骨架（四页路由 + 恢复页，见 ADR-002）；
...
5. 迁移公共库和头像；
6. 建立 React 后台应用（`@wordpress/scripts` 构建）、恢复页与 Site Health 输出；
...
| 小工具容器 | 首发；模块 `Services/Apps`，沙箱 iframe + 签名 manifest + 桥接 REST |
| 公告 | 固定源模块保留（wptea.com、one.weixiaoduo.com 经 wpcy.com 聚合） |
| 后台 | 整站 React + 恢复页（推翻原「四页原生 + 单页小应用」） |
| option 键名 | 不带版本：`wpcy_settings` 等；演进靠 `schema_version` |
