...
4.0 不应在 3.9 的 `Service/Setting.php` 和私有设置框架上继续改造，而应建立一个只保留“连接优化 + 文派托管 CDN 接入”的新插件内核。旧功能按迁移白名单接入，其余功能默认不移植。
...
**WP-China-Yes 是 WordPress 的中国连接优化客户端，也是文派托管 CDN 的站点接入端。**
...
不建议以隐藏本地开关、代码混淆或“Pro 文件包”收费。插件是 GPL 客户端，权限和配额由服务端账户、站点和套餐决定。
...
| 文派托管 CDN | 4.0 商业主线 | 管理 API、鉴权、配额、回滚和服务端控制面准备后启用 |
| WP-CLI | 建议首发 | 至少提供 status、doctor、config export/import、cdn purge |
| 遥测 | 只保留最小 opt-in | 与诊断上报分开同意，默认关闭，不默认发送站点 URL |
...
| 后台广告拦截、通知隐藏 | 删除；风险高且与连接无关 |
| 飞行模式/任意域名阻断 | 删除；如未来需要，单独做可审计的网络策略产品 |
...
| 白标/OEM 管理后台 | 不进 4.0 首发；代理商品牌属于服务端组织能力 |
...
2. **文派 CDN 服务页**：仅这一页使用 `@wordpress/components` 构建小型应用，通过自有 REST 路由读取账户、域名、用量和刷新任务。

建议菜单固定为四页：

- 概览：当前线路、异常和建议；
- 连接优化：WordPress.org、公共库、头像、可选字体；
- 文派 CDN：连接账户、域名、状态、刷新、用量；
- 诊断与隐私：检查结果、导出、遥测同意、重置。
...
| `telemetry` | `privacy.telemetry` | 只保留明确开启值 |
| `telemetry_site_url` | `privacy.include_site_url` | 必须依赖 telemetry 为 true |
...
这样可以删除为 WordPress 4.9、PHP 7.4 保留的大量历史分支。正式确定最低版本前，应统计现有安装版本分布；没有数据时不能直接宣称升级不会影响用户。
...
门禁：生产域名切换仍由人工控制台执行；故障时插件撤销改写并保留原站可访问。
...
| 4.0 是否继续做万能工具箱 | 否，只做连接优化和托管 CDN 接入 |
| 商业版是否另发 Pro 插件 | 否，同一 GPL 客户端，服务端 entitlement 收费 |
| 免费用户是否必须登录 | 否 |
| 是否自动修改 DNS | 4.0 首发不做 |
| 是否保留评论/维护/监控/白标 | 不保留 |
| Windfonts 是否进入首发 | 可选模块，API 目录合同完成后决定 |
| PHP/WordPress 最低版本 | 推荐 PHP 8.1、WordPress 6.5；发布前看安装分布确认 |
