...
规格条目编号供验收对照。任务书、实现对照表、视觉审查表都写编号，不写「概览那张卡」。
...
存量 §3 正文本身不改编号；下列编号只在本节登记，供对照表使用。后续规格修订任务若要把编号写进 §3 各条，另开任务。

| 编号 | 节 | 内容 |
|------|----|------|
| OV-01 | 3.1 A | 恢复模式横幅 |
| OV-02 | 3.1 B | 线路状态卡组（三卡） |
| OV-03 | 3.1 B | WordPress.org 源卡 |
| OV-04 | 3.1 B | 公共库与头像卡 |
| OV-05 | 3.1 B | 文派服务绑定卡 |
| OV-06 | 3.1 C | 需要处理 |
| OV-07 | 3.1 D | 公告 |
| OV-08 | 3.1 | 空状态：尚未检查 |
| CO-01 | 3.2 | 顶栏「保存」与成功 Snackbar「已保存」 |
| CO-02 | 3.2 | WordPress.org 源 |
| CO-03 | 3.2 | 公共前端库 |
| CO-04 | 3.2 | 头像 |
| CO-05 | 3.2 | 字体（Windfonts） |
| CO-06 | 3.2 | 多站点网络策略 / 子站覆盖 |
| SV-01 | 3.3 A | 未绑定 |
| SV-02 | 3.3 A | 绑定中 |
| SV-03 | 3.3 A | 已绑定 |
| SV-04 | 3.3 A | 服务端不可达 |
| SV-05 | 3.3 B | 权益与配额表 |
| SV-06 | 3.3 B | 未绑定空状态「绑定后显示」 |
| SV-07 | 3.3 C | 小工具网格与角标 |
| SV-08 | 3.3 C | 打开容器 |
| SV-09 | 3.3 C | 需权益 Modal |
| SV-10 | 3.3 C | 索引不可达 |
| DG-01 | 3.4 | 顶栏「立即检查」 |
| DG-02 | 3.4 | 连接检查 Tab |
| DG-03 | 3.4 | 被隐藏的通知 Tab |
| DG-04 | 3.4 | 出站主机记录 Tab |
| DG-05 | 3.4 | 数据与恢复 |
| RC-01 | 3.5 | 标题与说明 |
| RC-02 | 3.5 | 「关闭全部 URL 改写」 |
| RC-03 | 3.5 | 「停用全部模块」 |
| RC-04 | 3.5 | 已在恢复模式 |
| RC-05 | 3.5 | 「返回概览」 |
