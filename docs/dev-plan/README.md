...
| M2-11 | 桥接来源校验落地规格修订：PHP `Bridge::classify` 接受 `"null"` origin、`session_token` 会话令牌（JS+PHP+mock-app+e2e） | M2-10 | — | 见 `tasks/M2-11.md` | `composer check` 绿；分支 CI e2e 全绿；合同测试覆盖 token 缺失/错误/第三方导航场景 |
