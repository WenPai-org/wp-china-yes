...
| M-STATS-1 | 概览数据源（`tasks/M-STATS-1.md`）：本地计数器 10 个 + 事件日志 + `GET /stats` `GET /events` + `installed_at` + `profile_confirmed_at` + 浏览器测速允许名单补充。契约 `specs/rest-api.md` §`/stats` §`/events`。**2026-09-06 夜派 Grok（`grok/m-stats-1`）** | M-UI-0 | 不改 `src/Admin/**`、`src/Privacy/**` | 见任务书 | `composer check` + CI 全绿；DoD |
