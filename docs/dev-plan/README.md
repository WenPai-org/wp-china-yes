...
| M-PROVIDER-1 | 供应商层后端（`tasks/M-PROVIDER-1.md`）：薇晓朵商城连接（邮箱 + 密钥，加密独立 option）、已购产品、精确匹配的更新接通、`/providers*` REST；文派集市 `coming_soon` 占位。规格 `specs/providers.md`（据 R-PROVIDERS-0 研究 + 拍板 P1–P10）。**2026-09-06 夜派 Grok（`grok/m-provider-1`）**，与 M-STATS-1 并行 | M-BLOCK-1、`/binding` | 不改 `src/Admin/**`、`SiteBinding` 行为 | 见任务书 | `composer check` + CI 全绿；DoD |
