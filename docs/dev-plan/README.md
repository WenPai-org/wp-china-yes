...
<<<<<<< HEAD
| M-UI-0 ✅ | 产品化后端前置（`tasks/M-UI-0.md`）。**已合入**（2026-09-06；CI run 34021801112；审查无阻断。遗留给 M-SCOPE-1 顺带：`rollback()` 同时删 `wpcy_migration_report`；`ResidencyController::log_items` 再投影四字段；补 `source_version` 缺省 / 分页第二页 / 中文文案测试）：`/residency/*` 与 `/migration/report` REST、索引 `unconfigured` 态、面向用户错误串中文化、恢复页 title | M4-02 | 不改 `src/Admin/app/` | 见任务书 | `composer check` + CI 全绿；DoD |
