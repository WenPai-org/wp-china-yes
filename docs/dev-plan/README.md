...
| M2-10 | CI e2e 四条小工具用例（A3 定位器、A5 同源桥接 `data.set`、A8 索引不可达提示、A9 双层 iframe 超时），以 CI 为验证回路 | M2-09 | — | 见 `tasks/M2-10.md` | 已合入 `c8447af`；e2e 23/23（任务书误写 27） |
| M2-11 | 桥接来源校验落地规格修订：PHP `Bridge::classify` 接受 `"null"` origin、`session_token` 会话令牌（JS+PHP+mock-app+e2e） | M2-10 | — | 见 `tasks/M2-11.md` | 已合入；e2e 24/24（run 34002282877）；独立审查阻断（第二条 `ready` 重发 token）已修 |
...
| M-BLOCK-1 ✅ | HTTP Block 三层引擎（`tasks/M-BLOCK-1.md`）：L0 受保护主机 / L1 驻留 / L2 本站拦截清单 + 噪声包，`/site-blocklist`、`/residency/protected`、`/residency/test`。**已合入**（2026-09-06 夜；分支 CI run 34040789846 全绿；独立审查阻断「通用 PUT 可写 L0」由 M-BLOCK-1b 修；合并时统筹补一处：M-SCOPE-1c 新增的多站点 `DocumentWriter::put_site()` 同样拒绝 `modules.site_blocklist`；报告 `reports/M-BLOCK-1*`）。遗留：云桥 ingest 主机名常量为空待 devops；建议 4 / 6 未做（可选） | — | — | — | 已验 |
| M-STATS-1 ✅ | 概览数据源（`tasks/M-STATS-1.md`）：本地计数器 10 个 + 事件日志 + `GET /stats` `GET /events` + `installed_at` + `profile_confirmed_at` + 浏览器测速允许名单 + `Diagnostics\RouteGroups`（分组 / 服务商）。**已合入**（2026-09-07 01:5x；分支 CI run 34049237287 绿；审查两阻断由 M-STATS-1b 修：`mirror_fallbacks` 接到 `WordPressOrgModule` 回上游处、多站点 `installed_at` 读 site_option；数字诚实项：镜像只计 version-check 与安装包、心跳每分钟一次、头像只在 `get_avatar_url`；合并时统筹给 `HttpBlock` L2 与噪声包拦截点接上 `outbound_blocked`；报告 `reports/M-STATS-1*`）。**已知遗留**：跨境站 `wordpress_org=off` 时不记 `update_check` 事件（要不要在直连时也记，由 feibisi 定） | — | — | — | 已验 |
| M-PROVIDER-1 ✅ | 供应商层后端（`tasks/M-PROVIDER-1.md`）：薇晓朵商城连接（邮箱掩码 + 密钥 secretbox 独立 option）、`product_list` 校验、已购产品缓存、精确匹配的更新接通、`/providers*` REST、日志脱敏、`uninstall.php`；文派集市 `coming_soon`。**已合入**（2026-09-07 02:2x；分支 CI run 34050443293 绿；真机只读探测确定 `product_list` 校验 / 密钥只能 query / 邮箱不参与，已回写 `specs/providers.md` §4；审查阻断（`products()` 缓存门）由 1b 修；报告 `reports/M-PROVIDER-1*`）。遗留：`UrlGuard` 只拦字面量私网（建议 1 可选）；`activated_products[]` 键需补进规格 §2 | — | — | — | 已验 |
| M-UI-1 | 产品化 UI 第一步（`tasks/M-UI-1.md`）：壳（SH-01–08）+ 组件集 + 概览六态（OV-*）+ 恢复页（RC-*）。原型 `design/prototypes/e/`（认可 `APPROVAL.md` 2026-09-06）、规格 `admin-ui-spec.md` v2.0。**2026-09-06 夜暂缓**：feibisi 要求改 Hero / 来自文派 / 图标并加服务商与供应商（决定 `decisions/2026-09-06-core-services-value-and-providers.md`），原型出 v6、规格 v2.1 后再启动；与 M-STATS-1 并行、靠 REST mock 验收 | M-UI-0、原型认可 | 不改后端 | 见任务书 | 三层验收（`acceptance-sop.md`）+ 10 张视觉对照图 |
| M-UI-2 | 设置页（简单 / 高级 / 网络只读，CO-*）+ 向导四步（WZ-*）。依赖 M-UI-1 组件集。原型已有；切换场景 Modal / 保存失败 / 网络管理员视图三个状态待补原型 | M-UI-1 | — | 待写 | 三层验收 |
| M-UI-3 | 服务页（SV-*，**SV-11 可用服务目录数据源待定**，需服务端 `GET /services/catalog` 契约）+ 诊断页（DG-*，含 M-BLOCK-UI 的 CO-10 / DG-07）。依赖 M-UI-1、M-BLOCK-1、服务端契约 | M-UI-1、M-BLOCK-1 | — | 待写；先补原型 | 三层验收 |
