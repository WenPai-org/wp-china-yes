...
| `wp-china-yes-4.0.zip` | 本分支 `bash scripts/build-release.sh`（mapper 修改之后） | `8a9ff499dc8cfe853b8cc0b9779a428ba4b6a586d84f50962095521ed354277a` | 仓内文件名仍是 `dist/wp-china-yes-3.9.3.zip` |
...
2. 3.8 `admincdn=['admin']` 与 `admincdn_files` 含 `admin`：4.0 `public_assets` 均为 `[]`（`admin` 不在白名单；4.0 `AssetMap` 故意不含 wp-admin 改写）。
...
`wpcy_settings` 非空：`wordpress_org=off` `avatar=weavatar` `public_assets` 五项 schema 默认（`admincdn=[]` 无 token，不触发显式清空）。  
...
```
# 预置 3.8 ZIP + single-3.8-02.json
# 哈希 49b4f2acb853f8da560846b94e1ab419bd521d74859aa8d4fdd7be4fd2f6e0ed
# 键含 admincdn=["admin"]
```

dry-run：`kept=["store","cravatar","windfonts","adblock"]` `ignored` 11 项（`admincdn` 及 plane/memory/hide 等）。  
`wpcy_settings`：`wordpress_org=off` `avatar=cravatar_cn` **`public_assets=[]`**（3.8 `admincdn=['admin']` 与 3.9 `admincdn_files` 含 `admin` 同一 4.0 值）。  
eval_exit=0，前台 200。装回 3.8 / 3.9.3 哈希均为 `49b4f2ac…`。
...
dry-run kept 同 S1 四键。eval_exit=0，前台 200。装回 3.8 / 3.9.3 哈希 `4d03c20e…`。

### N2 多站点 3.8 · `multisite-3.8-05.json`

哈希 `a4371f71e595b2358d79044381748b6737eb64c30c805ba4eb4947b3a8010f6b`（四阶段相同）。  
`wpcy_network_settings` 非空；子站 `wpcy_settings` ABSENT_OK。`admincdn=[]` → `public_assets` 五项默认。前台 200。

### N3 多站点 3.8 变体 · `multisite-3.8-06.json`（`admincdn` 空串）

哈希 `d85413acd5e29c33f23f10b18439c8c4d48664eb16c0643e7ba63b9099ce78ee`（四阶段相同）。  
`admincdn=""` 无 token，与 N2 相同走 schema 默认五项。`wpcy_network_settings` 非空；子站 option 未写。前台 200。
...
dry-run ignored 含 `huge_blob`（`feature_removed`）。`wp_china_yes` 哈希四阶段均为 `cee7ac6a55c599941c1b171597fb2023cd9c44fcbf1334970865f7d3dd2e423c`，键仍含 `huge_blob`（**未被截断改写**）。eval_exit=0，前台 200。
...
| admincdn_files 含 `admin` 与 3.8 `admincdn` | 同一 4.0 值 | 两者 `public_assets` 均不含 `admin`；仅含 `admin` 时均为 `[]`（`Mappers.php:192-223,301+`） | 已做 |
| 无 admincdn_* 三键 | schema 默认五项 | 空 `admincdn=[]` / `""` 不触发；S1/N1/N2/N3 仍默认五项 | 已做 |
...
改了 PHP。`composer check` exit **0**（phpcs + phpstan 73/73 OK + 全部 PHPUnit suites，其中 Migration `OK (34 tests, 508 assertions)`）。  
`npm run build` exit 0；`npm run lint:js` exit 0（未改 JS）。

未改 `.github/workflows/ci.yml`。push `grok/m4-02` 用既有 CI 验证。
...
4. **Studio 矩阵未单独跑 `wp wpcy migrate --rollback`**：装回 3.x 不依赖 rollback。单元 `test_rollback_restores_without_rewriting_legacy` 覆盖「rollback 不写 `wp_china_yes`」。
...
- [x] 测试：`FixturesTest` 新增 5 个 §5 用例；`composer check` 0；`npm run build` 0。CI run id 见 push 后。
- [x] `git diff --stat` 允许路径内；无 `.grok-context/`；无 `docs/dev-plan/README.md`；无 fixtures JSON 数据改动。
- [x] 没做 / 疑问：上一节。
